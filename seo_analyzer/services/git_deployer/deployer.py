...
"""
    ...
    def deploy_sitemap(self, xml_content: str, commit_message: str = None) -> Dict:
        """
        Deploy sitemap.xml to Git repository

        Args:
            xml_content: Generated sitemap XML content
        ...
        try:
            # Step 1: Clone repository
            logger.info(f"Cloning repository for sitemap deployment: {self.domain.git_repository}")
            self._clone_repository()

            # Step 2: Write sitemap.xml file
            sitemap_path = self._write_sitemap_file(xml_content)
            logger.info(f"Wrote sitemap to: {sitemap_path}")

            # Step 3: Commit changes
            commit_hash = self._commit_sitemap(commit_message)
            logger.info(f"Committed sitemap with hash: {commit_hash[:8]}")

            # Step 4: Push to remote
            logger.info("Pushing sitemap to remote repository")
            self._push_to_remote()
            ...
            self._update_domain_error(f"Authentication error: {str(e)}")
            self._cleanup()
            return {
                'error': True,
                'message': f'Git authentication failed: {str(e)}'
            ...
            self._update_domain_error(str(e))
            self._cleanup()
            return {
                'error': True,
                'message': f'Git operation failed: {str(e)}'
            ...
            logger.error(f"Sitemap deployment error: {e}", exc_info=True)
            self._update_domain_error(f"Unexpected error: {str(e)}")
            self._cleanup()
            return {
                'error': True,
                'message': f'Sitemap deployment failed: {str(e)}'
            }

    def _write_sitemap_file(self, xml_content: str) -> str:
        """
        Write sitemap.xml to the appropriate location in the repository

        Args:
            xml_content: Sitemap XML content

        Returns:
            Path to the written file (relative to repo root)
        """
        repo_path = Path(self.temp_dir)

        # Determine sitemap location based on project structure
        target_path = self.domain.git_target_path or 'public'
        sitemap_dir = repo_path / target_path

        # Handle common static folder patterns
        possible_locations = [
            repo_path / target_path,  # Configured path (e.g., 'public')
            repo_path / 'public',      # Standard public folder
            repo_path / 'static',      # Django/Flask static
            repo_path / 'dist',        # Built output
            repo_path / 'out',         # Next.js static export
            repo_path,                 # Root (for some static sites)
        ]
        ...
        for loc in possible_locations:
            ...
            # If no directory exists, use the configured path and create it
            sitemap_dir = repo_path / target_path
        ...
        return str(sitemap_path.relative_to(repo_path))

    def _commit_sitemap(self, commit_message: str = None) -> str:
        """
        ...
        """
        # Stage sitemap.xml
        self.repo.git.add('sitemap.xml')
        # Also try to add from common locations
        try:
            self.repo.git.add('public/sitemap.xml')
        except:
            pass
        try:
            self.repo.git.add(A=True)  # Stage all changes as fallback
        except:
            pass
        ...
        if not commit_message:
            commit_message = f"Update sitemap.xml - {timestamp}"

        message = f"""{commit_message}

Sitemap updated by SEO Analyzer
...
"""
    ...
    def _update_domain_error(self, error_message: str):
        """Update domain with error status"""
        self.domain.deployment_status = 'failed'
        self.domain.last_deployment_error = error_message
        self.domain.save(update_fields=['deployment_status', 'last_deployment_error'])
